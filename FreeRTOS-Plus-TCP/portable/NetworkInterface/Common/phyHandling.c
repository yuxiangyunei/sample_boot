	...
	if( ulDoneMask != ( uint32_t)0u )
	{
		...
		uint32_t ulPhyID = pxPhyObject->ulPhyIDs[ xPhyIndex ];
			...
			FreeRTOS_printf( ( ">> Autonego ready: %08lx: %s duplex %u mbit %s status\n",
				...
				//eventLogAdd( "%s duplex %u mbit %s st", ( ulRegValue & phyPHYSTS_DUPLEX_STATUS ) ? "full" : "half", ( ulRegValue & phyPHYSTS_SPEED_STATUS ) ? 10 : 100, ( ( ulPHYLinkStatus |= phyBMSR_LINK_STATUS ) != 0) ? "high" : "low" );
{
	uint32_t regs[4];
	int i,j;
	int address = 0x10;
	for (i = 0; i < 4; i++)
	{
		...
		//eventLogAdd("%04lX %04lX %04lX %04lX", regs[0], regs[1], regs[2], regs[3]);
	}
}
