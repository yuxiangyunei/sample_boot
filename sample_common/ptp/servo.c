...
void initClock(PtpClock *ptpClock)
{
    ...
    netEmptyGeneralQ(&ptpClock->netPath);
}
                  ...
                  const TimeInternal *correctionField)
{
    ...
    if (0 != ptpClock->currentDS.offsetFromMaster.seconds)
    {
        ...
        //updateOffset: cannot filter seconds
        return;
    }
...
void updateDelay(PtpClock *ptpClock,
                 ...
                 const TimeInternal *recieveTimestamp,
                 const TimeInternal *correctionField)
{
    /* Tms valid ? */
...
void updatePeerDelay(PtpClock *ptpClock, const TimeInternal *correctionField, Boolean twoStep)
{
    ...
    {
        //updatePeerDelay: cannot filter with seconds
        return;
    }
    else
    {
...
volatile uint8_t ptp_adjust_flag = 0;
void updateClock(PtpClock *ptpClock)
{
    ...
    if (0 != ptpClock->currentDS.offsetFromMaster.seconds || abs(ptpClock->currentDS.offsetFromMaster.nanoseconds) > MAX_ADJ_OFFSET_NS)
    {
        ...
        if (!ptpClock->servo.noAdjust) //秒/100MS级别的修改
        {
    ...
    {
        if (abs(ptpClock->currentDS.offsetFromMaster.nanoseconds) < DEFAULT_UNCALIBRATED_OFFSET_NS)
        {
            if (0 == ptp_adjust_flag)
            {
                ptp_adjust_flag = 1;
            }
        }
        ...
        ptpClock->observedDrift += offsetNorm / ptpClock->servo.ai; //16
        ...
        if (!ptpClock->servo.noAdjust)
        {
            adj = (Integer32)((float)offsetNorm / (float)ptpClock->servo.ap + (float)ptpClock->observedDrift);
            //adj = (Integer32)((float)offsetNorm);
            adjFreq(-adj);
        }
        ...
        if (DEFAULT_PARENTS_STATS)
        {
            ...
            //updateClock: observed scalled log variance
        }
    }
}
