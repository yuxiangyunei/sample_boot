...
void initData(PtpClock *ptpClock)
{
    ...
    {
        //initData: UUID length is not valid");
    }
...
{
    return (Boolean)(0 == memcmp(A->clockIdentity, B->clockIdentity, CLOCK_IDENTITY_LENGTH) && (A->portNumber == B->portNumber));
}
                              ...
                              PtpClock *ptpClock)
{
    ...
    if (0 != grandmasterIdentityComp)
    {
        ...
        {
            //bmcDataSetComparison: grandmaster.Identity: B better then A
            return B_better_then_A;
        }
        else if (grandmasterIdentityComp < 0)
        {
            //bmcDataSetComparison: grandmaster.Identity: A better then B
            return A_better_then_B;
        }
    }
    ...
    {
        //bmcDataSetComparison: stepsRemoved: B better then A
        return B_better_then_A;
    }

    if ((announceB->stepsRemoved) > (announceA->stepsRemoved + 1))
    {
        //bmcDataSetComparison: stepsRemoved: A better then B
        return A_better_then_B;
    }
    ...
    if ((announceA->stepsRemoved) > (announceB->stepsRemoved))
    {
        ...
        {
            /* reciever < sender*/
            //bmcDataSetComparison: PortIdentity: B better then A
            return B_better_then_A;
        }
        else if (comp < 0)
        {
            /* reciever > sender */
            //bmcDataSetComparison: PortIdentity: B better by topology then A
            return B_better_by_topology_then_A;
        }
        else
        {
            ////bmcDataSetComparison: //ERROR 1
            return ERROR_1;
        }
    }
    else if ((announceA->stepsRemoved) < (announceB->stepsRemoved))
    {
        ...
        {
            /* reciever < sender*/
            //bmcDataSetComparison: PortIdentity: A better then B
            return A_better_then_B;
        }
        else if (comp < 0)
        {
            /* reciever > sender */
            //bmcDataSetComparison: PortIdentity: A better by topology then B
    ...
    {
        /* A > B*/
        //bmcDataSetComparison: sourcePortIdentity: B better by topology then A
        return B_better_by_topology_then_A;
    }
    else if (comp < 0)
    {
        /* B > A */
        ////bmcDataSetComparison: sourcePortIdentity: A better by topology then B
        return A_better_by_topology_then_B;
    }

    /* compare port numbers of recievers of A and B - same as we have only one port */
    //bmcDataSetComparison: //ERROR 2
    return ERROR_2;
}
...
UInteger8 bmc(PtpClock *ptpClock)
{
    ...
    {
        if ((bmcDataSetComparison(&ptpClock->foreignMasterDS.records[i].header, &ptpClock->foreignMasterDS.records[i].announce, &ptpClock->foreignMasterDS.records[best].header, &ptpClock->foreignMasterDS.records[best].announce, ptpClock)) < 0)
        {
            best = i;
        }
    }
